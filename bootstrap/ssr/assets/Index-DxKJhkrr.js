...
function Index({ auth, notes, filters = {} }) {
    ...
    Authenticated,
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsx("div", { className: "py-12", children: /* @__PURE__ */ jsxs("div", { className: "max-w-7xl mx-auto sm:px-6 lg:px-8", children: [
          ...
          filterData.groupBy ? Object.entries(notes.reduce((groups, note) => {
            const key = note[filterData.groupBy] ? filterData.groupBy === "project_id" ? note.project?.name : note.task?.title : "Ungrouped";
            if (!groups[key]) groups[key] = [];
            groups[key].push(note);
            return groups;
          }, {})).map(([groupName, groupNotes]) => /* @__PURE__ */ jsxs("div", { className: "mb-8", children: [
            /* @__PURE__ */ jsx("h3", { className: "font-bold text-lg text-gray-700 mb-4 px-2 border-l-4 border-indigo-500 bg-gray-50 py-1", children: groupName }),
            /* @__PURE__ */ jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: groupNotes.map((note) => /* @__PURE__ */ jsx(NoteCard, { note, handleDelete, handleDownload }, note.id)) })
          ] }, groupName)) : /* @__PURE__ */ jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: notes.length > 0 ? notes.map((note) => /* @__PURE__ */ jsx(NoteCard, { note, handleDelete, handleDownload }, note.id)) : /* @__PURE__ */ jsx("div", { className: "col-span-full text-center py-12 text-gray-500 bg-white rounded-lg border border-dashed border-gray-300", children: "No notes found. Create one to get started!" }) })
        ] }) })
      ]
    }
  );
}
const NoteCard = ({ note, handleDelete, handleDownload }) => /* @__PURE__ */ jsxs("div", { className: "bg-white overflow-hidden shadow-sm rounded-lg flex flex-col h-full border border-gray-100", children: [
